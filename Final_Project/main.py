...
from train import normal_train, cross_val_train

# Set up argument parser
if __name__ == '__main__':
    parser = argparse.ArgumentParser(description='Prediction of Pedestrian Speed with NN')
    parser.add_argument('--config', type=str, help='Path to YAML configuration file')
    parser.add_argument('--raw_data_dir', type=str, default=' ', help='Path to the raw data, only needs to be specified at the first time')
    ...
    if config["model"]["name"] == "NN":
        cross_val_train(config=config)